...
export class GiimallIndexedDB {
	constructor(options) {
		let { name = 'GiimallIndexedDB', version = 1, stores, use, timeout } = options || {}

		if (!stores || !Array.isArray(stores) || !stores.length) {
			stores = [
				{
					name: use || 'GiimallIndexedDB',
					isKeyValue: true,
				},
			]
		}
	...
	transaction(writable = false) {
		...
		const request = (runtime) => () => {
			return this.db().then((db) => {
				...
				tx.onabort = (e) => {
					runtime.resolve() // 中止已完成任务
					runtime.complete()
				}
				if (this.timeout > 0) {
					setTimeout(() => {
						if (runtime && runtime.status) {
							runtime.reject(new Error('[GiimallIndexedDB]: transaction timeout'))
							runtime.complete()
						}
					}, this.timeout)
				}
	...
	/**
	 * 创建IDB请求
	 * @param {function} prepare 使用当前objectStore返回请求
	 * @param {boolean} direct 是否返回请求self，如果为false，则返回请求成功结果，默认为false
	...
	remove(obj) {
		return this.keyPath().then((keyPah) => {
			...
			if (key === undefined) {
				throw new Error(`[GiimallIndexedDB]: your passed object to remove() does not contain keyPath '${keyPah}'.`)
			}
...
export default GiimallIndexedDB
