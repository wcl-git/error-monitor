...
export class ErrrorWeb {
    constructor(param) {
        ...
        this.stylize = this.stylize && ErrrorWeb._stylizeSupport();
    ...
    async clearHistory () {
        if (!this.isFile) {
            let history = await idb.getItem('ErrrorWeb');
            if (history && history.data.length) {
                this.queue.unshift(...history.data);
            }
            await idb.delete('ErrrorWeb')
        }
    }
    // 记录点击事件
    exceptionClick () {
    ...
    ajaxHandler() {
        if (this.autoLogAjax) {
            ...
            XMLHttpRequest.prototype.open = function (...args) {
                this._ErrrorWebMethod = args[0];
                this._ErrrorWebUrl = ErrrorWeb.resolveUrl(args[1]);
            ...
            XMLHttpRequest.prototype.send = function (data) {
                ...
                this.addEventListener('readystatechange', function () {
                    ...
                    if (that.logAjaxFilter(this._ErrrorWebUrl, this._ErrrorWebMethod)) {
                        try {
    ...
    //处理日志队列
    send () {
        if (this.queue && this.queue.length && this.sendStatus === ErrrorWeb.workerEnmu.ready) {
            this.saveError()
        }
    }
    ...
    saveError () {
        ...
        this.worker.onmessage = async (event) => {
            ...
            if (event.data === ErrrorWeb.workerEnmu.pending) {
                this.sendStatus = ErrrorWeb.workerEnmu.pending
            }

            // 上报成功, 清除队列
            if (event.data === ErrrorWeb.workerEnmu.done) {
                this.queue = [];
                this.retryCount = 1;
    ...
    _printConsole(time, level, ...args) {
        if (console && this.config.isLog) {
            if (this.stylize) {
                console[level](`%c[${Fn._getTimeString(time)}] [${level.toUpperCase()}] -`, `color: ${ErrrorWeb.colorEnum[level]}`, ...args);
            } else {
                console[level](...args);
            }
        }
    ...
    // 记录一条信息日志
    info(...args) {
        this._log(null, ErrrorWeb.levelEnum.info, ...args);
    }
    ...
    // 记录一条警告日志
    warn(...args) {
        this._log(null, ErrrorWeb.levelEnum.warn, ...args);
    }

    // 记录一条错误日志
    error(ajaxErrorJSON, ...args) {
        this._log(null, ErrrorWeb.levelEnum.error, ajaxErrorJSON, ...args);
    }
